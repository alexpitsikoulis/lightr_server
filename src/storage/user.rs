...
pub async fn upsert_user(db_pool: &PgPool, user: &User) -> Result<PgQueryResult, Error> {
    query(
        r#"
        INSERT INTO users (id, email, name, password, profile_photo, email_confirmed, created_at, updated_at, deleted_at, failed_attempts)
        ...
        DO
            ...
            WHERE
                (users.email, users.name, users.password, users.profile_photo, users.email_confirmed, users.deleted_at, users.failed_attempts) IS DISTINCT FROM
                (EXCLUDED.email, EXCLUDED.name, EXCLUDED.password, EXCLUDED.profile_photo, EXCLUDED.email_confirmed, EXCLUDED.deleted_at, EXCLUDED.failed_attempts)
        "#
    )
        .bind(user.id())
...
pub async fn get_user_by_id(db_pool: &PgPool, id: Uuid) -> Result<User, Error> {
    query_as(
        r#"
            SELECT id, email, name, password, profile_photo, email_confirmed, created_at, updated_at, deleted_at, failed_attempts
            FROM users
            WHERE id = $1
        "#
    )
