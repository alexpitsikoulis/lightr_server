...
async fn test_login_success() {
    ...
    let test_cases = vec![(
        format!(
            "email=testuser%40youwish.com&password={}",
            TEST_USER_PASSWORD
        ),
    ...
    for (body, error_message) in test_cases {
        let response = app
            .client
            .request(
                Path::POST(format!("{}{}", USER_BASE_PATH, LOGIN_PATH)),
...
async fn test_login_failure_on_invalid_credentials() {
    ...
    let test_cases = vec![
        (
            "email=testuser%40youwish.com&password=S0meotherpassword!".to_string(),
            "the email is found but the password is incorrect",
        ),
        (
            "email=someotheremail%40test.com&password=Testp@ssw0rd1".to_string(),
